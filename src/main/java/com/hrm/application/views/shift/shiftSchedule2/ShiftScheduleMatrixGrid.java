...
public class ShiftScheduleMatrixGrid extends Div {
    ...
    public ShiftScheduleMatrixGrid() {
        setSizeFull();
        initGrid();
        add(grid);
    }
    ...
    private Grid.Column<ShiftSchedulesQueryVO> buildDayColumn(LocalDate date, Map<String, Long> slotCounts) {
        ComponentRenderer<Div, ShiftSchedulesQueryVO> renderer = new ComponentRenderer<>(
                ...
                (cell, row) -> updateCell(cell, row, date)
        );
    ...
    private void onCellClick(Div cell) {
        ...
        String currentKey = getShiftTypeForDate(row, clickedDate);
        ...
        ShiftTypeSelectDialog dialog = new ShiftTypeSelectDialog(clickedDate, allTypes, current);
    ...
    private void updateCell(Div cell, ShiftSchedulesQueryVO row, LocalDate date) {
        ...
        String shiftTypeKey = (row == null) ? NO_ASSIGN : getShiftTypeForDate(row, date);

        Byte status = (row == null) ? null : getShiftStatusForDate(row, date);
        Byte actionType = (row == null) ? null : getActionTypeForDate(row, date);

        // 內容文字
        cell.setText(
                isLeave(status) ? "請"
                        : (NO_ASSIGN.equals(shiftTypeKey) ? NO_ASSIGN
                        : (shiftTypeKey != null && shiftTypeKey.contains("HOLIDAY") ? "假" : "班"))
        );

        // 顏色
        String bg = (row == null) ? "#ffffff" : getShiftColorCodeForDate(row, date);
    ...
    private void setUpHeadersForDate(Grid.Column<ShiftSchedulesQueryVO> col, LocalDate date) {
        ...
        // 星期
        String dowTxt = DOW_ZH_NUM[date.getDayOfWeek().getValue() - 1];
    ...
    // from VO 快取讀取
    private String getShiftTypeForDate(ShiftSchedulesQueryVO row, LocalDate date) {
        var m = indexRow(row);
        ShiftSchedulesDateTimeQueryVO sd = m.get(date);
        return (sd == null) ? NO_ASSIGN : sd.getShiftTypes();
    }
    private Byte getShiftStatusForDate(ShiftSchedulesQueryVO row, LocalDate date) {
        var m = indexRow(row);
        ShiftSchedulesDateTimeQueryVO sd = m.get(date);
        return (sd == null) ? null : sd.getStatus();
    }
    private Byte getActionTypeForDate(ShiftSchedulesQueryVO row, LocalDate date) {
        var m = indexRow(row);
        ShiftSchedulesDateTimeQueryVO sd = m.get(date);
        return (sd == null) ? null : sd.getActionType();
    }
    private String getShiftColorCodeForDate(ShiftSchedulesQueryVO row, LocalDate date) {
        var m = indexRow(row);
        ShiftSchedulesDateTimeQueryVO sd = m.get(date);
