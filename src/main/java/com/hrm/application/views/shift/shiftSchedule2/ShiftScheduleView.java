...
@Scope("prototype")
@Route(value = "shiftSchedule", layout = MainLayout.class)
public class ShiftScheduleView extends VerticalLayout {
    ...
    // grid
    private final ShiftScheduleMatrixGrid matrixGrid = new ShiftScheduleMatrixGrid();

    public ShiftScheduleView(ShiftScheduleService service) {
        ...
        // 個人排班模式
        matrixGrid.setMode(ShiftScheduleMatrixGrid.Mode.PERSONAL);
...
//    @Override
//    public MenuLink getPageLink() {
//        return MenuLink.SCHEDULING;
//    }
    ...
    private void updateSchedulesData() {
        ...
        // 顯示範圍（核心雙週 ±1 週）
        LocalDate displayStart = monthlyPeriods.get(0).getStartDate().minusWeeks(1);
        LocalDate displayEnd   = monthlyPeriods.get(monthlyPeriods.size() - 1).getEndDate().plusWeeks(1);
    ...
    private void onSaveAll() {
        ...
        try {
            ...
            List<ShiftSchedulePeriod> monthlyPeriods = service.getShiftSchedulePeriods(first.format(fmt), last.format(fmt));
            if (monthlyPeriods == null || monthlyPeriods.isEmpty()) {
                Notification.show("本月沒有可儲存的雙週區間");
                return;
            }
            LocalDate saveStart = monthlyPeriods.get(0).getStartDate();
            LocalDate saveEnd = monthlyPeriods.get(monthlyPeriods.size() - 1).getEndDate();

            Integer currentUserId = Optional.ofNullable(SessionUtil.getUserInfo())
                    .map(u -> u.getId()).orElse(null);
            if (currentUserId == null) {
                Notification.show("無法取得目前使用者資訊");
                return;
            }
            ...
            List<ShiftSchedules> payload = buildPayloadForSave(myRows, saveStart, saveEnd);

            if (payload.isEmpty()) {
                Notification.show("當前範圍內沒有可儲存的班表資料");
                return;
            }

            boolean ok = service.savePersonalShiftSchedules(payload);
            if (ok) {
                Notification.show("排班儲存成功");
                updateSchedulesData();
            } else {
                Notification.show("儲存失敗，請稍後再試");
            }
        } catch (Exception ex) {
            Notification.show("儲存失敗：" + ex.getMessage());
        } finally {
